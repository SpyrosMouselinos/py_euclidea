...
class Segment(Line):
    ...
    def dist_from(self, x):
        p1, p2 = self.end_points
        if np.dot(p2 - p1, x - p1) < 0:
            return np.linalg.norm(x - p1)
        elif np.dot(p1 - p2, x - p2) < 0:
            return np.linalg.norm(x - p2)
        else:
            return Line.dist_from(self, x)
    ...
    def closest_on(self, np_point):
        ...
        if self.contains(cand):
            ...
            return min(self.end_points, key=lambda x: np.linalg.norm(x - np_point))
...
class Ray(Line):
    ...
    def dist_from(self, x):
        if np.dot(self.v, x - self.start_point) >= 0:
            ...
            return np.linalg.norm(x - self.start_point)
